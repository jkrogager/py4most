...
def main():
    ...
    # Add quasars from z=4 to z=6
    z_range = np.random.uniform(4., 6., 1000)
    mag_range = np.random.uniform(18., 23., 1000)
    template = 'ROSAT_AGNT1-DR16QSO-stitched-stack-qmost-template.fits'
    temp_fname = os.path.join(TEMP_PATH, template)
    band = 'r'
    num = 1
    reddening = 0.
    for redshift, mag in zip(z_range, mag_range):
        temp_wl, temp_flux, redshift = load_template(
                                           temp_fname,
                                           redshift,
                                           mag, band,
