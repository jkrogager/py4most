...
TEMP_PATH = '/Users/krogager/Projects/4MOST/templates/opr25/stellar/'
...
def load_template(filename, mag, band, Ebv=0):
...
def save_mock_spectrum(filename, wl, flux, err, qual, t_exp, ra, dec, cname, uid, redshift=None, magnitude=None, fib_id=None, mag_filter=None, tempname=''):
    N = len(wl)
    hdu = fits.HDUList()
...
def apply_noise(temp_wl, temp_flux, sky_model, throughput, transmission,
                ...
                magnitude=None, mag_filter=None, tempname='', date_str=None, output_dir='output'):
    """
    ...
    filename = os.path.join(output_dir, cname+'_%s_1%05i_LJ1.fits' % (date_str, fibnum))
    save_mock_spectrum(filename, wl_joint, flux_joint, err_joint, qual_joint, t_exp,
                       ra=ra, dec=dec, cname=cname, uid=uid, redshift=redshift, fib_id=fibnum,
...
def main():
    ...
    parser.add_argument('input', type=str,
                        help='Path to Stellar Spectral Templates')
    ...
    # Load target catalog:
    cat = glob.glob(os.path.join(args.input, '*.fits')) * 10
    magnitudes = np.random.uniform(15, 21, len(cat))
    RA = np.random.uniform(0, 360, len(cat))
    DEC = np.random.uniform(-80, -10, len(cat))
    UID = np.arange(len(cat)) + 10100000
    ...
    for tname, mag, ra, dec, uid in zip(cat, magnitudes, RA, DEC, UID):
        # print(row['TEMPLATE'], row['CNAME'], row['MAGNITUDE'])
        tname = os.path.basename(tname)
        temp_fname = os.path.join(TEMP_PATH, tname)
        if 'SN' in temp_fname:
            continue
        band = 'r'
        reddening = 0.

        temp_wl, temp_flux  = load_template(
                                           temp_fname,
                                           mag, band,
                                           Ebv=reddening,
                                           )
        ...
        date_str = datetime.datetime.now().strftime("%Y%m%d")
        output_dir = f'/Users/krogager/Projects/4MOST/test_data/{date_str}/'
